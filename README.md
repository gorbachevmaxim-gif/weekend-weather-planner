...
3.  **Рекомендации по экипировке**:
    ...
    *   `temperature_2m` (Почасовая температура): Ключевой параметр. Вычисляются минимумы и максимумы за время поездки, а также сравниваются утренние (09:00-11:00) и дневные (11:00-18:00) значения для адаптивных советов (например, съемные рукава).
    *   `wind_speed_10m` (Скорость ветра): Максимальные значения ветра определяют класс ветрозащиты (плотная куртка или легкая жилетка).
    *   `precipitation` (Осадки): Влияют на сам факт рекомендации поездки.

    **Правила формирования гардероба:**
    *   **Торс (Джерси)**:
        *   Ниже +8°C: Зимняя куртка (Winter Jacket).
        *   +8°C...+15°C: Утепленная длинная джерси (Long Sleeve Jersey Cold).
        *   +15°C...+22°C: Легкая длинная джерси (Long Sleeve Jersey Hot).
        *   Выше +22°C: Короткая джерси (Jersey).
    *   **Ноги**:
        *   Ниже +14°C: Теплые рейтузы (Bib Tights).
        *   Выше +14°C: Шорты (Bib Shorts).
