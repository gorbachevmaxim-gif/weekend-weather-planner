...
export function getWeekendDates(): TargetDate[] {
    ...
    for (const hStr of HOLIDAYS) {
        ...
        if (hDate >= today && hDate <= rangeEnd) {
            const exists = targets.some(t => t.date.getTime() === hDate.getTime());
            if (!exists) {
                targets.push({ date: hDate, type: 'extra' });
            }
        }
    }
...
export async function analyzeCity(cityName: string, coords: CityCoordinates, targetDates: TargetDate[]): Promise<CityAnalysisResult | null> {
    if (!targetDates.length) return null;
    const startStr = toLocalISODate(targetDates[0].date);
    const endStr = toLocalISODate(targetDates[targetDates.length - 1].date);
    ...
    try {
        ...
        const baseTime = targetDates[0].date.getTime();

        const promises = targetDates.map(async (target, index) => {
            ...
            if (!hourly.precipitation || hourly.precipitation.length < eIdx) return null;
        ...
        const calculatedStats = await Promise.all(promises);
        
        calculatedStats.forEach(item => {
            if (!item) return;
            const { type, stats } = item;
            if (type === 'w1_sat') result.weekend1.saturday = stats;
            else if (type === 'w1_sun') result.weekend1.sunday = stats;
            else if (type === 'w2_sat') result.weekend2.saturday = stats;
            else if (type === 'w2_sun') result.weekend2.sunday = stats;
            else if (type === 'extra') result.extraDays.push(stats);
        });
        
        result.extraDays.sort((a, b) => a.dateObj.getTime() - b.dateObj.getTime());
